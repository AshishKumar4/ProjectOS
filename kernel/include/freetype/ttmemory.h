...
#include "freetype/ttconfig.h"
#include "freetype/tttypes.h"
#include <string.h>
...
#else
#define MEM_Copy( dest, source, count )  memcpy( source, dest, count )
#endif

#ifdef HAVE_MEMMOVE
#define MEM_Move( dest, source, count )  memmove( dest, source, count )
#else
#define MEM_Move( dest, source, count )  memmove( source, dest, count )
#endif
